...
* Windows shall be opened when the user clicks/double cliks an icon. The same icon should be represented in the upper bar of the window.
* The user shall be able to drag and move the windows in the document.
* The user shall be able to open and close new windows of the desired application.
...
You connect to the server via Websockets and send messages using the json format:
...
### The API-key
To get the api-key to the chat application please log in och the [profile page](https://coursepress.lnu.se/kurs/klientbaserad-webbprogrammering/course-information/profile-information/) on the course web page.
We use this key to keept track of users of the application.

### Functional requirements Chat application:
* The user should be able to open several chat applications at once.
...
* The user should be able to see the 100 latest messages since the chat applications was opened.
* One, by you decided, extended feature.
...
### Basic hints
* This assignment can look big at first glance. Please take an hour or two using pen and paper to sketch up your application visually aswell as structure your code. Should you make use of types? In that case, which types can you identify?
...
* Your applications are isolated units. Can you make them in such a way that they are independent of your PWD-application?
